...
key = urandom(16)
"""
keeping this prefix length for testing if our guess of prefix len will be right
...
def has_duplicate(blocks):
    return len(blocks) > len(set(blocks))


def determine_prefix_length(block_size=16):
    """
        to determine the length of prefix we first take guess length of atleast 2* blocksize  
    """
    guess_length = 2*block_size

    while True:
        blocks1 = getblocks(oracle(bytes([0]*guess_length)), block_size=16)
        blocks2 = getblocks(oracle(bytes([1]*guess_length)), block_size=16)

        if has_duplicate(blocks1) and has_duplicate(blocks2):
            break

        guess_length += 1
    blocks = getblocks(oracle(bytes([0]*guess_length)), block_size=16)
    first_block_index = [
        i * block_size for i in range(len(blocks)-1) if blocks[i] == blocks[i+1]][0]
    return first_block_index-(guess_length-2*block_size)


def oracle(input):
    return ecb_encrypt(PREFIX+input+secret, key=key)
...
if __name__ == "__main__":
    ...
    plain_text = b""
    prefix_length = determine_prefix_length()
    assert prefix_length == PREFIX_LEN, "prefix length algorithm not working as expected"

    prefix_padding_length = block_size-prefix_length % block_size
    prefix_padding = bytes([0x00]*prefix_padding_length)
    no_of_prefix_blocks = (prefix_length + prefix_padding_length) // block_size
    ...
    for _ in range((len(oracle(bytes())))):
        ...
        ciphertext = oracle(prefix_padding+pad)
        prefix = last_known_block[1:]
        actual_block = getblocks(ciphertext, block_size)[
            no_of_prefix_blocks+target_block_index]
        for byte in range(256):
            guess = getblocks(
                oracle(prefix_padding+prefix+bytes([byte])), block_size)[no_of_prefix_blocks]
            match_found = False
